...
const { restaurantDB, likeDB } = require('../../../db')
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    detail.tags = tagArray

    const data = { info, detail }

    // 성공적으로 users를 가져왔다면, response를 보내줍니다.
    res.status(statusCode.OK).send(util.success(statusCode.OK, responseMessage.READ_RESTAURANT_SUCCESS, data));
